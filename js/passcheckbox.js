...
async function getDataEvents() {
    ...
    let arrayPass = datosEventos.filter(e => dataFecha > e.date)
    ...
    function displayCheckBox() {
        ...
        var allCategory = arrayPass.map(eventoCheck => eventoCheck.category)
    ...
    function arraySearch() {
        ...
        if (selectedCheck.length > 0 && wrintingSearch !== "") {
            selectedCheck.map(categorys => {
                searchData.push(...arrayPass.filter(eventos => eventos.name.toLowerCase().includes(wrintingSearch.trim().toLowerCase()) &&
                    eventos.category == categorys))

            })

        } else if (selectedCheck.length > 0 && wrintingSearch === "") {
            selectedCheck.map(categorys => searchData.push(...arrayPass.filter(eventos => eventos.category == categorys)))
        } else if (selectedCheck.length == 0 && wrintingSearch !== "") {
            searchData.push(...arrayPass.filter(eventos => eventos.name.toLowerCase().includes(wrintingSearch.trim().toLowerCase())))
        } else {
            searchData.push(...arrayPass)
        }
         ...
         <p class="card-text">${eventoHome.description}</p>
          ...
          <div class="card-footer">
            ...
            <p class="card-text">$${eventoHome.price}</p>
                ...
                document.getElementById('mainCardPass').innerHTML = templateCard

            });

        } else {
            document.getElementById('mainCardPass').innerHTML = templateCard = `<p>No hay ningun resultado</p>`
        }
